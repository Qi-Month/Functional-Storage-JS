...
public class DrawerBuilder {
	...
	private DrawerBuilder(String name) {
		this.name = name;
	}

	public static DrawerBuilder of(String name) {
		return new DrawerBuilder(name);
	}

	@Info("Sets the log block used by the drawer")
	public DrawerBuilder log(Block block) {
		log = getBlockId(block, "log");
		return this;
	}

	@Info("Sets the planks block used by the drawer")
	public DrawerBuilder planks(Block block) {
		planks = getBlockId(block, "planks");
		return this;
	}

	@Info("Sets the base front texture path. The 1x1, 1x2 and 2x2 models use the path with _1, _2 and _4 appended. Defaults to functional_storage_js:block/<name>_front, which renders as the missing texture when no png is provided.")
	public DrawerBuilder frontTexture(ResourceLocation texture) {
		frontTexture = texture;
		return this;
	}

	@Info("Sets the side texture path. Defaults to functional_storage_js:block/<name>_side, which renders as the missing texture when no png is provided.")
	...
	private static ResourceLocation parse(String id, String kind) {
		...
		try {
			...
			ConsoleJS.STARTUP.warn("Drawer %s block id is invalid: " + id, e);
			return null;
		}
	}
